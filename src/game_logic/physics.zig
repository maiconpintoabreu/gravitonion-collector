...
pub const PhysicsSystem = struct {
    ...
    pub fn resetById(self: *PhysicsSystem, id: i32) void {
        if (self.physicsBodyList[@as(usize, @intCast(id))]) |body| {
            body.isColliding = false;
        }
    }

    pub fn reset(self: *PhysicsSystem, tag: PhysicsBodyTagEnum) void {
        for (self.physicsBodyList) |optionalBody| {
    ...
    pub fn addBody(self: *PhysicsSystem, body: *PhysicsBody) i32 {
        ...
        rl.traceLog(.info, "Body Added: %i", .{body.id});
    ...
    fn checkCollisions(self: *PhysicsSystem) void {
        for (&self.physicsBodyList, 0..) |leftOptionalBody, i| {
            if (leftOptionalBody) |leftBody| {
                if (!leftBody.enabled) continue;
                for (&self.physicsBodyList, 0..) |rightOptionalBody, j| {
                    if (rightOptionalBody) |rightBody| {
                        if (i == j) continue;
